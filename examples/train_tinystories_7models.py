...
# Принудительно UTF-8 в stdout/stderr (для эмодзи и кириллицы под Windows cp1251)
try:
...
from src.layers.hypercube_linear_layers import (
    BaseHypercubeLinear,
    BinaryHypercubeLinear,
    TernaryHypercubeLinear,
    TwoBitHypercubeLinear,
)
...
# ПЛАН ЭКСПЕРИМЕНТА
# ==========================================
# Обучение 7 моделей на сабсете TinyStories:
#   1. FP32 (плотный baseline)
#   2. Бинарная гиперкуб  (латентное сжатие + BinarySTE)
#   3. Тернарная гиперкуб (латентное сжатие + TernarySTE)
#   4. 2-битная гиперкуб  (латентное сжатие + TwoBitSTE)
#   5. Бинарная + обычный STE   (плотные веса, BinarySTE, без латента)
#   6. Тернарная + обычный STE  (плотные веса, TernarySTE, без латента)
#   7. 2-битная + обычный STE   (плотные веса, TwoBitSTE, без латента)
...
# Объем сабсета TinyStories в символах.
# ~1.5M символов -> компактный и быстрый прогон 7 моделей.
TINYSTORIES_SUBSET_CHARS = 1_500_000
...
# ==========================================
# ЗАГРУЗКА И ПОДГОТОВКА ДАННЫХ (TinyStories)
# ==========================================
def load_tinystories_subset(n_chars: int, cache_path: str = None):
    """
    Загружает датасет TinyStories (через HuggingFace `datasets`) и берёт
    подстроку первых n_chars символов конкатенированных историй.
    Кэширует результат в файл, чтобы не скачивать повторно.
    Имя кэш-файла включает размер, чтобы разные сабсеты не конфликтовали.
    """
    if cache_path is None:
        cache_path = f"tinystories_subset_{n_chars}.txt"
    if os.path.exists(cache_path):
        print(f"📂 Загружаю кэшированный сабсет TinyStories из {cache_path}")
        with open(cache_path, 'r', encoding='utf-8') as f:
            return f.read()

    print("⏳ Скачиваю TinyStories через HuggingFace datasets...")
    ...
    for ex in ds:
        ...
        if not text:
            continue
        # Разделитель между историями для читаемости границ
        text = text.strip() + "\n"
        chunks.append(text)
        total += len(text)
        if total >= n_chars:
    ...
    with open(cache_path, 'w', encoding='utf-8') as f:
        f.write(full)
    print(f"✅ Сохранён сабсет ({len(full)} символов) в {cache_path}")
    return full


def build_dataset(text: str):
    """Символьная токенизация (как в существующих примерах)."""
...
def model_configs(vocab_size: int):
    """Возвращает список (имя, linear_cls, group_size, latent_dim) для 7 моделей."""
    cfg = dict(
    ...
    return [
        ...
        # Гиперкубы (латентное сжатие + STE)
        ("Binary_Hypercube", lambda: build(BinaryHypercubeLinear, 8, LATENT_DIM)),
        ("Ternary_Hypercube", lambda: build(TernaryHypercubeLinear, 5, LATENT_DIM)),
        ("TwoBit_Hypercube", lambda: build(TwoBitHypercubeLinear, 5, LATENT_DIM)),
        # Обычный STE (плотные веса, без латентного сжатия)
        ("Binary_STE", lambda: build(BinarySTELinear, None, None)),
        ("Ternary_STE", lambda: build(TernarySTELinear, None, None)),
...
def model_weight_bits(model: nn.Module):
    """
    Эвристическая оценка 'битности' хранимых весов:
      - nn.Linear (FP32 baseline)         -> 32 бита
      - QuantizedLinearSTE (плотные STE)  -> битность по типу квантования (1/2/2)
      - BaseHypercubeLinear               -> латент хранится в FP32,
                                            группа из group_size весов описана
                                            latent_dim числами -> биты/вес =
                                            32 * latent_dim / group_size
    Эмбеддинги и LayerNorm всегда FP32 (не квантуются).
    """
    quant_bits = {
        BinarySTELinear: 1,
        TernarySTELinear: 2,   # 3 состояния -> ~1.585 бит, округлим до 2 (log-storage)
        TwoBitSTELinear: 2,
        BinaryHypercubeLinear: None,
        TernaryHypercubeLinear: None,
        TwoBitHypercubeLinear: None,
    }
    ...
    for name, mod in model.named_modules():
        if isinstance(mod, nn.Linear) and not isinstance(mod, QuantizedLinearSTE):
            ...
            # Латентное хранилище: weight_latent + proj_matrix хранятся в FP32
            # Эффективно описывают pad_features весов.
...
# ЦИКЛ ОБУЧЕНИЯ
# ==========================================
def train_one_model(name, build_fn, vocab_size, decode, run_dir):
    set_seed(SEED)
    print(f"\n{'=' * 60}")
    print(f"🚀 Модель: {name}")
    print(f"{'=' * 60}")
    ...
    with torch.no_grad():
        for _ in range(2):
            ...
            tokens = model.generate(context, max_new_tokens=120, temperature=1.0)[0].tolist()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="TinyStories: 7-model quantization comparison")
    parser.add_argument("--subset_chars", type=int, default=TINYSTORIES_SUBSET_CHARS,
                        help="Размер сабсета TinyStories в символах")
    parser.add_argument("--iters", type=int, default=MAX_ITERS)
    parser.add_argument("--out", type=str, default="runs/tinystories_7models",
    ...
    text = load_tinystories_subset(args.subset_chars)
    ...
    print(f"\nЗапускаю {len(configs)} моделей на {MAX_ITERS} итераций каждая...")

    results = []
    for name, build_fn in configs:
        try:
            res = train_one_model(name, build_fn, vocab_size, decode, args.out)
            results.append(res)
